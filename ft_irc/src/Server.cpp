...
/*   Updated: 2025/07/15 19:28:58 by librahim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int ac, char *av[])
{
    ...
    while (true)
    {
        ...
        if (poll_fds.at(0).revents & POLLIN)
        {
            ...
            register_cl(&poll_fds, client_fd);
        }
        for (int i = 1; i <= size_cl; i++)
        {
                ...
                {
                    memset(buf, 0, 512);
                    bytes_readen = recv(poll_fds.at(i).fd, &buf, sizeof(buf), 0);
                    if (bytes_readen > 0)
                    {
                        std::cout << "received message from client "<< i <<" : " << buf << std::endl;
                        std::string reply = "Welcome to ft_irc!\r\n";
                        send(poll_fds.at(i).fd, reply.c_str(), reply.length(), 0);
                    }
                }
        }
    }
